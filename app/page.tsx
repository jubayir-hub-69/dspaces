...
// CSS for Wallet Adapter to ensure dropdown works
...
export default function Home() {
  ...
  return (
    <main className={`min-h-screen transition-colors duration-500 relative overflow-hidden font-sans ${isDark ? 'bg-[#030712] text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      {/* Smart Navbar (z-50 ensures dropdown works) */}
      <nav className={`relative z-50 flex justify-between items-center px-4 sm:px-8 py-5 border-b backdrop-blur-md ${isDark ? 'border-white/5 bg-black/20' : 'border-gray-200 bg-white/40'}`}>
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 bg-clip-text text-transparent drop-shadow-sm cursor-pointer hover:scale-105 transition-transform">
          dSpaces
        </h1>
        ...
        <div className="flex items-center gap-4">
          ...
          {!isAuthenticated ? (
            <div className={`hidden sm:block px-4 py-1.5 rounded-full border text-xs font-semibold ${isDark ? 'border-gray-800 bg-gray-900/50 text-gray-400' : 'border-gray-300 bg-gray-100 text-gray-600'}`}>
              Not Connected
            </div>
          ) : connected ? (
      ...
      {/* Main Content (z-10 ensures it stays behind the dropdown) */}
          ...
          {!isAuthenticated ? (
            <div className="flex flex-col gap-6">
              ...
              <div className={`group text-left p-5 rounded-2xl border transition-colors ${isDark ? 'bg-gray-900/40 border-gray-800 hover:border-indigo-500/50' : 'bg-gray-50 border-gray-200 hover:border-indigo-400'}`}>
                <label className={`flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest mb-3 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                  <span className="w-2 h-2 rounded-full bg-indigo-500"></span>
                  Option 1: Web3 Wallet
                </label>
                <div className="flex justify-center w-full group-hover:scale-[1.02] transition-transform duration-300 relative z-40">
              ...
              <div className="flex items-center opacity-40">
                <div className={`flex-grow border-t ${isDark ? 'border-gray-600' : 'border-gray-400'}`}></div>
                <span className={`px-4 text-xs font-bold uppercase tracking-widest ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>OR</span>
                <div className={`flex-grow border-t ${isDark ? 'border-gray-600' : 'border-gray-400'}`}></div>
              </div>
                  ...
                  {!otpSent ? (
                    <div className="flex flex-col gap-2">
                      <input
                        ...
                        className={`w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${isDark ? 'bg-gray-950/50 border border-gray-700 text-white placeholder:text-gray-600' : 'bg-white border border-gray-300 text-gray-900 placeholder:text-gray-400'}`}
                      />
                      <button
                    ...
                    <div className="flex flex-col gap-2 animate-fade-in-up">
                      <div className="flex justify-between items-center mb-1">
                        <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Code sent to <span className={isDark ? 'text-white' : 'text-black'}>{email}</span></span>
                        <button onClick={() => { setOtpSent(false); setOtp(""); }} className="text-xs text-blue-500 hover:text-blue-400 underline">Change</button>
                      </div>
                      <input
                        type="text"
                        placeholder="• • • • • •"
                        ...
                        className={`w-full px-4 py-3 rounded-xl text-center text-xl tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-green-500 transition-all ${isDark ? 'bg-gray-950/50 border border-gray-700 text-white' : 'bg-white border border-gray-300 text-gray-900'}`}
                      />
                      <button
            ...
            <div className="flex flex-col gap-5 animate-fade-in-up">
              ...
              <div className="mb-2">
                ...
                <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Set your name and create or join a secure room.</p>
              </div>

              <div className="text-left">
                <label className={`text-[11px] font-bold uppercase tracking-widest ml-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Display Name</label>
                <input
                  type="text"
                  placeholder="e.g. John Doe"
                  value={userName}
                  onChange={(e) => setUserName(e.target.value)}
                  className={`w-full px-4 py-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 mt-2 transition-all ${isDark ? 'bg-gray-900 border border-gray-700 text-white placeholder:text-gray-600' : 'bg-gray-50 border border-gray-300 text-gray-900 placeholder:text-gray-400'}`}
                />
              </div>
              
              <div className="text-left">
                <label className={`text-[11px] font-bold uppercase tracking-widest ml-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Room ID (To Join)</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-3 w-full mt-4">
                ...
                <button 
                  ...
                  className={`py-4 font-bold rounded-xl transition-all border active:scale-[0.98] flex items-center justify-center gap-2 ${isDark ? 'bg-gray-800 hover:bg-gray-700 text-white border-gray-700 hover:border-gray-500' : 'bg-white hover:bg-gray-50 text-gray-800 border-gray-300 hover:border-gray-400'}`}
                >
